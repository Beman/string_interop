...
public:
       ...
       u32_t, std::input_iterator_tag, const u32_t>
  {
    ...
    typedef char source_value_type;

    BOOST_STATIC_ASSERT(sizeof(source_value_type)*CHAR_BIT == 8);
    BOOST_STATIC_ASSERT(sizeof(u32_t)*CHAR_BIT == 32);
    
    const source_value_type*  m_begin;
    const source_value_type*  m_end;

  public:

    // end iterator
    from_iterator() : m_begin(0) {}

    // by_null
    from_iterator(const source_value_type* begin) : m_begin(begin), m_end(begin) 
    {
      for (;
           *m_end != source_value_type();
           ++m_end) {}
      if (m_begin == m_end)
        m_begin = 0;
    }
    ...
    // by range
    template <class T>
    from_iterator(const source_value_type* begin, T end,
      // enable_if ensures 2nd argument of 0 is treated as size, not range end
      typename boost::enable_if<boost::is_same<const source_value_type*, T>, void >::type* x=0)
    ...
    // by_size
    from_iterator(const source_value_type* begin, std::size_t sz)
    ...
    {
      BOOST_ASSERT_MSG(m_begin, "Attempt to dereference end iterator");
      unsigned char c = static_cast<unsigned char>(*m_begin);
      return static_cast<u32_t>(interop::detail::to_utf16[c]);
    }

    bool equal(const from_iterator& that) const { return m_begin == that.m_begin; }

    void increment()
    { 
      BOOST_ASSERT_MSG(m_begin, "Attempt to increment end iterator");
      ++m_begin;
      if (m_begin == m_end)
        m_begin = 0;
    }
...
//  template <class ForwardIterator>
//  class to_iterator
//   : public boost::iterator_facade<to_iterator,
//       char, std::input_iterator_tag, const char>
//  {
//     typedef boost::iterator_facade<to_iterator,
//       char, std::input_iterator_tag, const char> base_type;
//   
//     typedef typename std::iterator_traits<ForwardIterator>::value_type base_value_type;
//
...
public:
  ...
  typedef typename FromCodec::value_type     from_value_type;
  typedef typename ToCodec::template to_iterator<from_iterator_type> to_iterator_type;
  typedef typename ToCodec::value_type       to_value_type;

  conversion_iterator() BOOST_DEFAULTED

  conversion_iterator(const from_value_type* begin)
    : to_iterator_type(from_iterator_type(begin)) {}

  template <class U>
  conversion_iterator(const from_value_type* begin, U end,
    // enable_if ensures 2nd argument of 0 is treated as size, not range end
    typename boost::enable_if<boost::is_same<const from_value_type*, U>, void >::type* x=0)
    : to_iterator_type(from_iterator_type(begin, end)) {}

  conversion_iterator(const from_value_type* begin, std::size_t sz)
